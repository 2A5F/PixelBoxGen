...
namespace PixelBoxGen
{
    ...
    #region BoxGenFrameEditor
    [CustomEditor(typeof(BoxFrame))]
    class BoxGenFrameEditor : Editor
    {
        public ReorderableList reorderableList;
        public int next_select;
        public SerializedProperty select;
        ...
        void OnEnable()
        {
            ...
            reorderableList = new ReorderableList(serializedObject, prop)
            {
                ...
                drawElementCallback = (rect, index, isActive, isFocused) =>
                {
                    ...
                    var select_val = select.intValue;
                    var Collider_rect = new Rect(rect)
                    {
                        y = rect.y + 45,
                        width = rect.width - 70,
                        x = rect.x + 70,
                    ...
                    if(EditorGUI.ToggleLeft(Collider_rect, "Selected", select_val == index))
                    {
                        if(select_val != index)
                        {
                            next_select = index;
        ...
        public override void OnInspectorGUI()
        {
            ...
            next_select = select.intValue;

            reorderableList.DoLayoutList();

            select.intValue = next_select;
    ...
    #region BoxGenFrameDrawer
    [CustomPropertyDrawer(typeof(ColliderImageFrame))]
    public class BoxGenFrameDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            using (new EditorGUI.PropertyScope(position, label, property))
